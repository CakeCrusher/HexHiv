...
const Leaderboard = (props) => {
    ...
    players.forEach((p, ind) => {
        ...
        rankings.push(<MapText text={`${ind + 1}.) ${p.points}`} x={0} y={ind * pixiSizeScalar(30)} fill={rgbToHex(p.color)} strokeFill={strokeFill} bgColor='rgb(0,0,0)' fontSize={pixiSizeScalar(18)} key={p.id} />)
    })
    return (
